...
var buildConditions = function(where){
	...
	var checkArray = function(sep, statements){
		...
		for(var i in statements)
			out.push(checkStatement(statements[i]));
			...
			switch(i){
				...
				default:
					if(statement[i] instanceof Object)
						out.push( checkStatement(statement[i], i) );
...
module.exports = {
	...
	builder: function(command, from, properties, where, sort, offset, limit){
		...
		console.log("BUILDER", command + " " + selectSql + " FROM " + from + whereSql + sortSql + ( offset ? " SKIP " + offset : "" ) + ( limit ? " LIMIT " + limit : "" ));
