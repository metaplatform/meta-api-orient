...
var buildConditions = function(where){
	...
	var processValue = function(val){
		...
		if(val instanceof Object)
			return checkStatement(val, null);
		else
			...
			switch(i){
				...
				case '$eq':
					...
					if(statement[i] === null)
						...
						out.push(sanitizeColumn(key) + " = " + processValue(statement[i]));
					break;
				...
				case '$match':
					if(!key) throw new Error("Invalid query syntax.");
					out.push(sanitizeColumn(key) + " MATCHES " + processValue(statement[i]));
					break;

				case '$like':
					if(!key) throw new Error("Invalid query syntax.");
					out.push(sanitizeColumn(key) + " LIKE " + processValue(statement[i]));
					break;

				case '$range':
					if(!key || !statement[i].from || !statement[i].to) throw new Error("Invalid query syntax.");
					out.push(sanitizeColumn(key) + " BETWEEN " + processValue(statement[i].from) + " AND " + processValue(statement[i].to));
					break;

				case '$instanceof':
					if(!key) throw new Error("Invalid query syntax.");
					out.push(sanitizeColumn(key) + " INSTANCEOF " + processValue(statement[i]));
					break;

				case '$in':
					if(!key) throw new Error("Invalid query syntax.");
					var inValues = [];
					for(var v in statement[i]) inValues.push(processValue(statement[i][v]));
				...
				case '$nin':
					if(!key) throw new Error("Invalid query syntax.");
					var ninValues = [];
					for(var w in statement[i]) ninValues.push(processValue(statement[i][w]));
				...
				default:
					if(statement[i] instanceof Object){
						...
						out.push(sanitizeColumn(i) + " = " + processValue(statement[i]));
